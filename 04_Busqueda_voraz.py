...
class Graph:
    ...
    def greedy_search(self, start, goal, heuristic):
        """
        Realiza una búsqueda voraz (Greedy Best-First Search) usando una función heurística.
        `heuristic` es un diccionario que asigna a cada nodo un valor estimado al objetivo.
        Retorna el camino encontrado o None si no existe.
        """
        visited = {start}
        parent = {start: None}
        # Cola de prioridad basada en heurística
        queue = [(heuristic.get(start, float('inf')), start)]
        ...
        while queue:
            ...
            for neighbor in self.adj_list.get(current, []):
                if neighbor not in visited:
                    ...
                    heapq.heappush(queue, (heuristic.get(neighbor, float('inf')), neighbor))
...
if __name__ == "__main__":
    ...
    # Definir heurística estimada al objetivo 'G'
    heuristic = {
        'A': 5,
        'B': 4,
        'C': 2,
